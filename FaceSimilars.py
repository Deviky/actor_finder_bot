...
def findMostSimilarFace(isActor, imageFileUploaded):
    dbService = DataBase()
    ...
    try:
        ...
        margin = 100
        ids = dbService.getAllActorIds() if isActor else dbService.getAllActressIds()

        for id in ids:
            print(id)
            currentRow = dbService.getActorById(id) if isActor else dbService.getActressesById(id)
            image = objectStorage.getImgActor(str(currentRow[2])) if isActor else objectStorage.getImgActress(str(currentRow[2]))
        ...
        if most_similar_file and most_similar_face_location:
            image = objectStorage.getImgActor(most_similar_file) if isActor else objectStorage.getImgActress(most_similar_file)
