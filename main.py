...
async def answerToUser(data, chat_id):
    ...
    if most_similar_photo!=None:
        await bot.send_photo(
            chat_id=chat_id, caption=f"Вы похожи на {most_similar_photo[1]}",
            photo=FSInputFile(path=most_similar_photo[0]), reply_markup=ReplyKeyboardMarkup(
            keyboard=[
                    [
                        KeyboardButton(text=textButton),
                    ]
                ],
        ...
        await bot.send_message(chat_id=chat_id, text="На вашем фото лицо не обнаружено(")
...
async def startFind(callback_query: CallbackQuery, state: FSMContext)->None:
    ...
    btn1 = InlineKeyboardButton(text="Актёр", callback_data="Actor")
    btn2 = InlineKeyboardButton(text="Актриса", callback_data="Actresse")
    row = [btn1, btn2]
    rows = [row]
    markup = InlineKeyboardMarkup(inline_keyboard=rows)
...
async def uploadPhoto(callback_query: CallbackQuery, state: FSMContext)->None:
    bol = None
    if callback_query.data == "Actor":
        bol = True
    elif callback_query.data == "Actresse":
        bol = False
    else:
        await callback_query.message.delete()
        btn1 = InlineKeyboardButton(text="Актёр", callback_data="choose")
        btn2 = InlineKeyboardButton(text="Актриса", callback_data="choose")
        row = [btn1, btn2]
        rows = [row]
        markup = InlineKeyboardMarkup(inline_keyboard=rows)
    ...
    await callback_query.message.delete()
    if bol is not None:
        await state.update_data(CHOOSING_TYPE=bol)
        await state.set_state(ActorSearchStates.PHOTO_UPLOAD)
        await callback_query.message.answer(
            "Загрузите ваше фото", reply_markup=ReplyKeyboardRemove()
        )
